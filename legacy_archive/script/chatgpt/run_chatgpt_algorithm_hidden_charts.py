#!/usr/bin/env python3
"""CHATGPT Algorithm Hidden Charts Runner

Loads the latest raw results from:
  data/results/chatgpt/algorithm_hidden/raw_results/
...
def generate_all_charts(platform: str, condition: str):
    ...
    charts.create_three_charts(results, timestamp, output_dir=charts_dir, file_prefix=file_prefix)

    summary_file = charts_dir / f"{condition}_charts_summary_{timestamp}.txt"
    summary_file.write_text(
        "\n".join([
            "ALGORITHM HIDDEN CRYPTANALYSIS - CHARTS GENERATED",
            "=" * 60,
            "",
            ...
            f"  1. algorithm_hidden_algorithm_identification_{timestamp}.html",
            f"  2. algorithm_hidden_performance_by_algorithm_{timestamp}.html",
            f"  3. algorithm_hidden_performance_trend_{timestamp}.html",
            "",
        ]) + "\n",
        encoding="utf-8",
    )
...
def generate_specific_chart(platform: str, condition: str, chart_type: str):
    ...
    charts = AlgorithmHiddenPerformanceCharts()
    df = charts._prepare_performance_data(results)

    if chart_type == "identification":
        charts.create_algorithm_identification_chart(df, timestamp, output_dir=charts_dir, file_prefix=file_prefix)
        print(f"Generated: algorithm_hidden_algorithm_identification_{timestamp}.html")
    elif chart_type == "performance":
        charts.create_performance_by_algorithm_chart(df, timestamp, output_dir=charts_dir, file_prefix=file_prefix)
        print(f"Generated: algorithm_hidden_performance_by_algorithm_{timestamp}.html")
    elif chart_type == "trend":
        charts.create_performance_trend_chart(df, timestamp, output_dir=charts_dir, file_prefix=file_prefix)
        print(f"Generated: algorithm_hidden_performance_trend_{timestamp}.html")
    else:
        raise ValueError("Unknown chart type. Use: identification, performance, trend")


def main():
    parser = argparse.ArgumentParser(description="Generate algorithm-hidden performance charts")
    parser.add_argument("--chart", type=str, help="Generate a specific chart (identification, performance, trend)")
