#!/usr/bin/env python3
"""
Run ChatGPT Algorithm Informed Experiment
...
class ChatGPTInformedAlgorithmExperiment:
    ...
    def _select_sample_cases(self, test_cases: List[Dict], max_tests: int) -> List[Dict]:
        """Pick a deterministic-ish sample with good algorithm coverage."""
        if max_tests <= 0:
            return []
    ...
    async def run_experiment(self, max_tests: int | None = None):
        """Run the informed algorithm experiment with ChatGPT"""
        print("\n" + "=" * 70)
        print("CHATGPT INFORMED ALGORITHM CRYPTANALYSIS EXPERIMENT")
        ...
        async with self.chatgpt_analyst as analyst:
            ...
            for batch_num in range(total_batches):
                ...
                for i, (test_case, chatgpt_response) in enumerate(zip(batch, batch_results)):
                    evaluation = self.evaluator.evaluate_cryptanalysis(chatgpt_response, test_case)
                ...
                self._save_raw_snapshot(batch_num + 1)
    ...
    def _save_raw_snapshot(self, batch_num: int):
        """Write an incremental raw-results snapshot so runs can be resumed safely."""
        snapshot_file = str(self.raw_dir / f"{self.platform}_{self.condition}_raw_results_{self.experiment_timestamp}_partial.json")
        payload = {
            'metadata': {
                'experiment': 'Chatgpt Cryptanalysis (partial)',
        ...
        print(f"Raw snapshot saved: {snapshot_file}")
    ...
    def _calculate_metrics(self):
        ...
        successful_responses = sum(1 for r in self.results if r['chatgpt_response']['success'])
    ...
    def _generate_reports(self):
        ...
        with open(results_file, 'w', encoding='utf-8') as f:
            json.dump({
                'metadata': {
                    'experiment': 'Chatgpt Cryptanalysis',
                    'timestamp': self.experiment_timestamp,
                    'test_cases_count': len(self.results),
                    'configuration': {
    ...
    def _generate_csv_report(self):
        ...
        for result in self.results:
            test_case = result['test_case']
            evaluation = result['evaluation']
            response = result.get('chatgpt_response', {})
    ...
    def _print_experiment_summary(self):
        ...
        print("\n" + "=" * 60)
        print("CHATGPT CRYPTANALYSIS EXPERIMENT SUMMARY")
        print("=" * 60)
...
async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Run ChatGPT algorithm_informed experiment")
    parser.add_argument('--max-tests', type=int, default=None, help='Run a sample of up to N tests (preflight)')
    ...
    print("Starting ChatGPT informed algorithm experiment...")
    
    try:
        experiment = ChatGPTInformedAlgorithmExperiment(api_key)
        results, metrics = await experiment.run_experiment(max_tests=args.max_tests)
        
        print(f"\nExperiment completed successfully!")
        print(f"ChatGPT analyzed {len(results)} ciphertexts with algorithm information.")
